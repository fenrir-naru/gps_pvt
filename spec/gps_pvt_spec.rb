...
__RINEX_OBS_TEXT__
  ...
  describe GPS::Solver do
    ...
    it 'calculates position without any error' do
      ...
      expect(sn.iono_utc.alpha).to eq([1.3970E-08, 2.2352E-08, -1.1921E-07, -1.1921E-07])
      expect(sn.iono_utc.beta).to eq([1.1059E+05, 1.6384E+05, -6.5536E+04, -5.2429E+05])
      
      meas.each{|prn, k, v|
        eph = sn.ephemeris(prn)
        expect(eph.iodc).to eq({31 => 78, 24 => 90, 12 => 90, 25 => 43, 18 => 0, 29 => 73}[prn])
        expect(eph.iode).to eq({31 => 78, 24 => 90, 12 => 90, 25 => 43, 18 => 0, 29 => 73}[prn])
      }
      ...
      meas.each{|prn, k, v|
        ...
        expect(solver.gps_options.excluded).to eq([prn])
        pvt2 = solver.solve(meas, t_meas)
        expect(pvt2.position_solved?).to eq(true)
